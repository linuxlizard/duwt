...
def find_test_function(src, fn_name):
    ...
    found = itertools.takewhile( lambda m : not m[0].startswith(s), 
                                zip(src, itertools.count()))
...
def main():
    ...
    if test_fn:
        ...
        new_test = []
        new_test.append("static void test_{}(const struct IE_HE_MAC* mac)\n{{\n".format(testname))
    ...
    try:
        job = subprocess.run("./test_ie_he", check=True, shell=False, 
                                    capture_output=True)
#                                    stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    job = subprocess.Popen("./test_ie_he", shell=False, 
                            bufsize=0,
